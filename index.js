...
const pkey = '23f2cde4101d8837d9f6f467097e99f4a93287db53e3d3d8e4123f66da32db2b563a' // token holder pkey

const contractAddress = '0x382cdce9d9dfa51cf99e6163c7f3a90b47c6384f' // token contract address
    ...
    if (bData.length === 0) {
      ...
      if (joinCheck) {
        ...
        if (('inviter' in pData[0]) && !('referred' in dbData[0])) {
          ...
          var see = cal + sen
          var bot_cur = 'tom'
          bot.telegram.sendMessage(pData[0].inviter, '➕ New Referral on your link you received 1000 DON', { parse_mode: 'markdown' })
          ...
          ctx.replyWithMarkdown(
            `*🎉 I am your friendly `+airdropName+` Airdropbot*\n\n*✅ Please complete all the tasks and submit details correctly to be eligible for the airdrop*\n\n💠 Join *`+airdropName+`* Telegram [Group](https://t.me/MetaDon_Group) and [Channel](https://t.me/METADON_OFFICIAL)\n💠 Follow *`+airdropName+`* on [Twitter](https://twitter.com/Poly_Drop), Like and Retweet pinned post\n💠 Subscribe to *`+airdropName+`* [Youtube](https://youtu.be/H6LOcbBhy5o)\n💠 Join *promoter* [Channel](https://t.me/Dora_Airdrop) and [Channel](https://t.me/PolyDrop)\n\n*ℹ️ You must complete all task then click "Submit My Details" button\n\n👉 Must Join Payment Channel: @METAPAYDON*`, { reply_markup: { keyboard: [['📘 Submit my details']], resize_keyboard: true } }
          )
        } else {
          db.collection('joinedUsers').insertOne({ userId: ctx.from.id, join: true })
...
bot.hears('📨 Information',async (ctx) => {
  ...
  ctx.replyWithMarkdown('*Token Information:*\n\n*Name:* `METADON`\n*Symbol:* `DON`\nDecimal: `18`\n*Contract Address:* `0x4Da1acDDb7692b42d07Eb06701228Fd4096679B5`\n\n_If you submitted a wrong data then you can restart the bot and start resubmission by clicking /start before Token airdrop ends._')
  })


bot.hears('📘 Submit my details', ctx => {
  var kkc = `🔹Join *`+airdropName+`* Telegram [Channel](https://t.me/METADON_OFFICIAL) And [Group](https://t.me/MetaDon_Group)
  
👉Join Payment Channel: @METAPAYDON\nJoin Promoter Channel1: @Dora_Airdrop
...
let joinCheck = await findUser(ctx)
  if(joinCheck){
    ...
    await ctx.replyWithMarkdown(`🔹Follow *`+airdropName+`* on [YouTube](https://youtu.be/H6LOcbBhy5o), Like and Subscribe Vedio 

*Submit your YouTube profile link *(Example: https://youtu.be/H6LOcbBhy5o)`, { disable_web_page_preview: true, reply_markup: { remove_keyboard: true } })
 } else {
    ctx.replyWithMarkdown('*You Must Join our channel& Promoter Channels*')
  }
})
...
ctx.scene.leave();
//await ctx.replyWithMarkdown(`*Submit your TomoChain Address from Token Pocket or Trust Wallet*`, { disable_web_page_preview: true, reply_markup: { remove_keyboard: true } })

await ctx.replyWithPhoto({ url: `https://telegra.ph/file/908921654fa294de73795.jpg` }, { caption: "Submit your Tomo Chain address ( Not exchange)" })
...
let tgData = await bot.telegram.getChatMember('@METADON_OFFICIAL', ctx.from.id) // user`s status on the channel
    let subscribed
...
function rndInt(min, max){
  ...
  let msg = `<b>📃 Dora Don Giveaway Bot Welcomes You

💵 Total For Airdrop : 1M Don
📡 Website : https://www.dhddhdhd
⌛️ Distribution : 15th March
